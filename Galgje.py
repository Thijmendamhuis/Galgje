...
def toon_geheimwoord():
    woord_label.config(text=f"Het woord was: {geheimwoord_str.lower()}")
    woord_label.grid(padx=100, pady=0)
...
def update_display(event=None):
    ...
    woord_label.grid_forget()


# Functie om een nieuw spel te starten
def nieuwe_game(event=None):
    global punten, levens, geraden_letters, geheimwoord_str
...
def gok(event=None):
    ...
    if gok not in geraden_letters:
        if levens >= 1:
            ...
            if gok.isalpha() and len(gok) == 1:
                ...
                if check_win():
                    ...
                    levens = 10
                    scoreboard()
                    # Toon de knop voor het starten van een nieuw spel
        ...
        tk.messagebox.showwarning("Waarschuwing", "Je heb deze lette al geprobeerd") 
        return None
...
woord_label = tk.Label(root, text="")
